...
import { ConstractUpload } from 'lib/components/ContractDocments/contractUseupload';
...
const UploadDoc = ({
  ...
  const upload = ConstractUpload({
    initialState: file && {
      ContractID: file.ContractID || regNo,
      issueDate: file.issueDate,
      expiryDate: file.expiryDate,
      SignedIDdate: file.issuePlace,
  ...
  React.useEffect(() => {
    if (upload.fomrdatavalue) {
      const loadurl = `${upload?.fomrdatavalue?.path}/download/${_.get(
        upload,
        'fomrdatavalue.id',
        ''
      )}`;
    ...
    if (upload.fileobj) {
      let x = upload?.fileobj?.name;

      var f = x.substr(0, x.lastIndexOf('.'));
      setfileName(f);
    }
  }, [upload.fomrdatavalue, upload.fileobj]);
                  ...
                  <Grid container spacing={5} xs={12}>
                    <Grid item xs={3}>
                      <TextField
                        ...
                        value={
                          attachmentID
                            ? attachmentID
                            : upload.details['ContractID']
                        }
                        fullWidth
                        label="CONTRACT ID"
                        disabled={attachmentID ? true : false}
                      />
                    </Grid>
                    <Grid item xs={3}>
                      <KeyboardDatePicker
                    ...
                    <Grid item xs={3}>
                      <KeyboardDatePicker
                        ...
                        label="SIGN  OFF DATE"
                        disablePast
                        value={upload.details['SignedIDdate']}
                        onChange={(e) => {
                          upload.updateDetails({
                            target: {
                              name: 'SignedIDdate',
