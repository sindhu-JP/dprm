...
const ProductDetails = ({
  ...
  console.log(Product.value, "properdata")
  React.useEffect(() => {
    let primaryDetails = {
    ...
    Product.setValue(values?.customerInfo?.PartnerProfileModification);
    console.log(values?.customerInfo?.PartnerProfileModification?.PartnerDetails, "product value", values?.customerInfo?.PartnerProfileModification?.PartnerDetails
    )
  }, [productData]);
  ...
  const PdfPreview = (url, item) => {
    ...
    if (url?.mimeType?.includes('application/pdf')) {
  ...
  const loadPDF = (dataURL) => {
    fetch(`${contractMangement}/${'fileuploads'}/${dataURL}`, {
      method: 'get',
      headers: {
  ...
  const getfilePreview = (item, title) => {
    ...
    //   getAuthorizedImage(item);
    // }
    if (getFile === 'pdf' || item?.mimeType?.includes('application/pdf')) {
      return (
  ...
  console.log(Product?.value, "produceeet")
      ...
      <Paper elevation={0}>
        {/* <Box p={4}> */}
          ...
          <Grid container spacing={4} direction="column">
            {!(_isEmpty(Product?.value)) &&   Object.keys(Product?.value).map((title) => {   
                          ...
                          <Grid container spacing={4}>
                            {Product.value &&
                              Object.keys(
                                ...
                                console.log(item, "itemaxine", Product.value[title])             
