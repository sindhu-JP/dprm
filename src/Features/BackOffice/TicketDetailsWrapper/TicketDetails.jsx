...
import BillingAdjustmentDetails from 'Features/TaskDetails/BillingAdjustment';
import CommissionRuleDetail from 'Features/TaskDetails/CommissionRuleDetails/CommissionRuleDetail';
import SettlementRuleView from 'Features/TaskDetails/SettlementRuleDetails/SettlementRule';
import _isEmpty from 'lodash/isEmpty';
import ModifyProduct from 'Features/TaskDetails/ProductDetails/modifyProduct';
...
const Settlement = ({ values, maintitle }) => {
        ...
        >
            ...
            <Grid container direction="row" spacing={2}>
              <Grid item>
                <Chip
                  ...
                  label={
                    !_isEmpty(values) &&
                    values?.MainObj[0]?.AddContractFor?.ContractInformation
          ...
          <Grid item xs>
            {values?.relatedEntity && (
              ...
              >
                {values?.relatedEntity[0]?.id?.substring(0, 2) === 'ST' ? (
                  <>
                ...
                ) : values?.relatedEntity[0]?.id?.substring(0, 2) === 'CM' ||
                  values?.relatedEntity[0]?.id?.substring(0, 2) === 'CO' ? (
                  <>
                    <Box className={classes.commBox}>
                      <CommissionRuleDetail
                        maintitle={'Commission Rule Details'}
                        commissionRuleDetails={values.MainObj}
                      />
                      {/* <CommissionRules values={values} /> */}
                    </Box>
                  </>
                ) : values?.relatedEntity[0]?.id?.substring(0, 2) === 'MP' ? (
                  <>
                    {/* <ProductDetails
                      maintitle={' Partner Information'}
                      productData={values.MainObj[0]?.PartnerProfileCreation}
                    /> */}
                  </>
                ) : values?.relatedEntity[0]?.id?.substring(0, 2) === 'TP' ? (
                  <>
                    <ProductDetails
                ...
                ) : values?.relatedEntity[0]?.id?.substring(0, 2) === 'PR' ? (
                  <></>
                ) : values?.relatedEntity[0]?.id?.substring(0, 2) === 'CR' ? (
                  <></>
                ) : values?.relatedEntity[0]?.id?.substring(0, 2) === 'AD' ? (
                  <></>
                ) : null}

                {values?.TaskMerger?.filter(
                  ...
                  //   formIdentity,
                  //   'lets see o',
                  //   formIdentity[item?.formIdentity]?.title,
                  //   item[formIdentity[item?.formIdentity]?.code],
                  //   item?.formIdentity
                  // );
                  if (item['@baseType'] === 'AdjustmentService') {
                    return (
                      <Box className={classes.commBox}>
                        <BillingAdjustmentDetails BillingAdjustment={item} />
                      </Box>
                    );
                  } else if (item['@baseType'] === 'SettlementRulesService') {
                    return (
                      <card className={classes.settBox}>
                        <SettlementRuleView
                          maintitle={'Settlement Rule Details'}
                          settlementRuleDetails={[item]}
                        />
                      </card>
                    );
                  } else if (item['@baseType'] === 'CommissionRules') {
                    return (
                      <Box py={4} className={classes.commBox}>
                        <CommissionRuleDetail
                          maintitle={'Commission Rule Details'}
                          commissionRuleDetails={[item]}
                        />
                      </Box>
                    );
                  } else {
                    return (
                      <div>
                        {!_isEmpty(
                          values?.customerInfo?.PartnerProfileModification
                        ) && (
                          <>
                            <Box>
                              <h1></h1>
                            ...
                            <ModifyProduct
                              ...
                              contractPreview={
                                item?.formIdentity === 'Add_Contract'
                                  ? true
                                  : false
                              }
                              productData={
                        ...
                        <>
                          <ProductDetails
                            preview
                            maintitle={formIdentity[item?.formIdentity]?.title}
                            // contractPreview={contractPreview}
                            formIdentity={item?.formIdentity}
                            contractPreview={
                              item?.formIdentity === 'Add_Contract'
