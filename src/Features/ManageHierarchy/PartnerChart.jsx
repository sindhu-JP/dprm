...
import ProductNewIcon from 'Assets/Icons/ProductNewIcon.svg';
...
const PartnerChart = (props) => {
  ...
  const getHierarchy = async (partnerId) => {
    TecnotreedigitalSales &&
      (await TecnotreedigitalSales.get(
        `/auth/secure/getuserhierarchybyparent/${partnerId}`
      )
        .then((resp) => {
          let nodeData = [];
  ...
  useEffect(() => {
    ...
    // Approved template
    OrgChart.templates.approved = Object.assign({}, OrgChart.templates.ula);

    OrgChart.templates.approved.node =
      '<rect x="0" y="0" rx="1" ry="10" height="120" width="250" fill="#ffffff" stroke-width="1" stroke="#00000029"></rect>' +
      '<line rx="1" ry="10" x1="0" y1="0" x2="250" y2="0" stroke-width="5" stroke="#30D973"></line>';

    OrgChart.templates.approved.nodeMenuButton =
      ...
      '</g>';
    OrgChart.templates.approved.link =
    ...
    OrgChart.templates.approved.field_0 =
        ...
        dispatch(
          Modal.open({
            ...
            context: {
              ...
              user: user,
              message: 'Product Created successfully, Approval pending'
            }
          })
        );
    ...
    const navigatetoAddTenant = (nodeId) => {
      setFormIdentity('Tenant_Partner_Profile');
      var nodeData = chart.get(nodeId);
      const relatedNode = filterByMaster(nodeData);
      setSelectedNodeData(relatedNode);
      // setDrawer(true);
        ...
        dispatch(
          Modal.open({
            ...
            context: {
              ...
              partnerlist:
                props.location?.state?.parterList?.partnerDetails
                  ?.PartnerDetails
            }
          })
        );
    ...
    var chart = new OrgChart(document.getElementById('tree'), {
      ...
      nodeMenu: {
        ...
        call_2: {
          icon: Notes,
          text: '<span>Add Product Contract</span>',
          onClick: navigatetoAddContract
        },
        call_3: {
          icon: Notes,
    ...
    <Paper elevation={0}>
      <Box style={{ display: 'flex', justifyContent: 'flex-end' }}>
        <Box style={{ padding: '0px 15px 0px 0px', display: 'flex' }}>
          ...
          />
          <Typography variant="h5">Partner Profile </Typography>
        </Box>
        <Box style={{ padding: '0px 15px 0px 0px', display: 'flex' }}>
          <img
            src={TenantPartner}
            ...
            style={{ marginRight: '5px' }}
          />
          <Typography variant="h5">Tenant</Typography>
        </Box>
      </Box>
      {/* <div id="legend">
