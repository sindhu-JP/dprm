...
import PartnerDetails from 'Features/360/Agent360/PartnerDetails/PartnerDetails';
...
const Partner360 = (props) => {
  ...
  React.useEffect(() => {
    ...
    if (props.location?.state?.detail) {
      ...
      if (details?.formIdentity === 'Dealers') {
         getUnsettlementBal(details?.ResellerProfileCreation?.DealerDetails?.Wallet_ID)
        PartnerRes.DynamicMenulist(constant.MenuList);
        // props.DcmProductlist({ id: mainlist.partnerId });
        showTabs.setValue(
          FilterOutOpco(
            PartnerRes.DynamicMenulist(constant.MenuList, 'agent'),
            details
          )
        );
        setTabname('AgentDetails');
        getPartnerId.setValue(mainlist.agentId );
        hideTenant.setTrue();
  ...
  console.log(props.DashboardDetails, "props here")
