...
const PartnerDetails = (props) => {
  ...
  React.useEffect(() => {
    if (props.partnerdetails) {
      ...
      _.map(
        details?.PartnerProfileCreation?.sections ||
          details?.TenantProfileCreation?.sections ||
          details?.ResellerProfileCreation?.sections ||
          details?.AddProduct?.sections,
        (item) => {
          temp.push({
            name: item,
            sectionId: item,
      ...
      <Grid item xs={9} className={classes.overFlow}>
        <Grid container spacing={6} className={classes.overFlow}>
          {props.partnerdetails.details?.PartnerProfileCreation &&
            props.partnerdetails.details.PartnerProfileCreation?.sections?.map(
              (item, index) => {
                return (
                  <Grid item xs={12}>
